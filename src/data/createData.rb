...
def separateGearByBrand(data, brand_list, category)
  ...
  brand_list.each do |brand|
    ...
    file_dir = "./#{category}/#{category}_#{brand['brand']}.json"
...
def createData file
  brand_list = JSON.parse(File.open('./originalData/brandList.json').read)
  data = []
  ...
  puts ''
  FileUtils.cp('./originalData/brandList.json', './brands.json')
  File.write("./#{category}.json", data.to_json.gsub!("\xEF\xBB\xBF", ''), encoding: 'UTF-8')
  puts "#{category} Data parsed and created!"
end

headgearCSV = './originalData/headgear.csv'
clothingCSV = './originalData/clothing.csv'
shoesCSV = './originalData/shoes.csv'
